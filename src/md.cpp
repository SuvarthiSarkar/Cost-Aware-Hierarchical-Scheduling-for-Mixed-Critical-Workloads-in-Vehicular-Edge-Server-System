// MD: Nearest BS, min-freq. No dropping, no D_max, no U_max.
// Wu et al. (IoT J. 2020)
#include "common.h"
    ...
    while(t<=MX){sim_collect(t,i,tasks,in);sim_complete(t,m,p,c);
        if(t%BATCH==0&&!in.empty()){sort(in.begin(),in.end(),comp_priority);vector<Task>ns;
            for(auto&x:in){if(x.process_time+t+1>x.deadline){if(x.flag==0)c.drophard++;else c.dropsoft++;continue;}
                ...
                if(can_fit(u,nr,ut,st,et,1.0)){x.start_time=st;x.end_time=et;assign_task(x,nr,u,p);c.add_distance(x,nr);}
                else ns.push_back(x);
            }in=ns;}t++;sim_energy(t,m,u,c);}
    for(auto&x:in){if(x.flag==0)c.drophard++;else c.dropsoft++;}print_stats_if_needed("MD",c);}
int main(int argc,char*argv[]){if(argc<2){cerr<<"Usage: ./md <input> [--detailed]\n";return 1;}
    check_detailed_flag(argc,argv);vector<Task>tasks;int m=read_input(argv[1],tasks);run(tasks,m);return 0;}
