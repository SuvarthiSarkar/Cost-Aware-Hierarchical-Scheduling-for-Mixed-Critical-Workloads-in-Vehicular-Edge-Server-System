// NEAREST: Nearest BS with deadline check, min-freq. No dropping, no D_max, no U_max.
#include "common.h"
void run(vector<Task>&tasks,int m){
    ...
    for(auto&x:in){if(x.flag==0)c.drophard++;else c.dropsoft++;}print_stats_if_needed("NEAREST",c);}
